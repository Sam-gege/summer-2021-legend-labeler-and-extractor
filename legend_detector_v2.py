...
def text_split(x, y, w, h, gap_thres, results, symbols, symbol_color, image, bw, right_split, criteria, txt_h=None):
    text_locs = []
    for i in range(len(results['text'])):
        conf = results['conf'][i]
        txt = results['text'][i]
    ...
    ######
    xt, wt = x, w
    left_gap = min(text_locs, key=lambda n: n[0])[0]
    rightmost = max(text_locs, key=lambda n: n[0] + n[2])
...
def text_narrow(x, y, w, h, bw, results, height_diff_thres):
    ## narrow text
    y_min = min(results['loc'], key=lambda n: n[1])[1]
    y_max_box = max(results['loc'], key=lambda n: n[1] + n[3])  # use max text height instead of bounding box height
    ...
    return min((y, h), (y1, h1), key=lambda n: n[1])
...
def symbol_text_convert(mask_bw, symbols, image, original_img, text_color, symbol_color, txt_h,
    ...
    symbols1 = []
    symbols.sort(key=lambda n: n[2] * n[3])
...
def symbol_search(rgb, texts, symbols, mask_x, mask_y, box_size, direction, search_box_color):
    '''
    ...
    i = 1
    w_max = max(texts, key=lambda n: n[2])[2]
    w_s, perc = box_size
    for x, y, w, h, txt in texts:
        # ignore if text is certain single characters
        if txt in {'A', 'a', '/', '&', '(', ')', '*'}:
            i += 1
            continue
        # delete text that has vertical or horizontal lines cross
        if direction == 'left' or direction == 'right':
            mask_slice = mask_y[y:y + h, x:x + w]
        elif direction == 'up':
            mask_slice = mask_x[y:y + h, x:x + w]
        else:
            raise ValueError('wrong direction.')
        pos = cv2.findNonZero(mask_slice)
        if pos is not None:
        ...
        # change search box dimensions to stop at the vertical or horizontal line
        if direction == 'left':
            mask_slice = mask_y[top:bottom, left:right]
            pos = cv2.findNonZero(mask_slice)
            if pos is not None:
                right_most = max(pos, key=lambda i: i[0, 0])[0]
                left += right_most[0]
            ...
            if pos is not None:
                left_most = min(pos, key=lambda i: i[0, 0])[0]
                right = left + left_most[0] + 1
        elif direction == 'up':
            mask_slice = mask_x[top:bottom, left:right]
            pos = cv2.findNonZero(mask_slice)
            if pos is not None:
                bottom_most = max(pos, key=lambda i: i[0, 1])[0]
                top += bottom_most[1]
        ...
        if pos is not None:
            ...
            if txt not in text_symbol.keys():
                ...
                if direction == 'left':  # if search direction is left, choose the symbol with largest x
                    text_symbol[txt] = max((x_old, y_old, w_old, h_old), (x, y, w, h), key=lambda n: n[0])
                elif direction == 'right':
                    text_symbol[txt] = min((x_old, y_old, w_old, h_old), (x, y, w, h), key=lambda n: n[0])
                elif direction == 'up':  # if search up, choose the symbol with largest y
                    text_symbol[txt] = max((x_old, y_old, w_old, h_old), (x, y, w, h), key=lambda n: n[1])
    ...
    for (x, y, w, h) in symbols_filtered:
        ...
        if pos is not None:
            ...
            if len(text_indices) > 1:
                ...
                max_left_diff = max(txts, key=lambda n: n[0])[0] - min(txts, key=lambda n: n[0])[0]
                max_center = max(txts, key=lambda n: n[0] + n[2] // 2)
                min_center = min(txts, key=lambda n: n[0] + n[2] // 2)
    ...
    '''loop through all symbols again, delete symbol-text pairs that crosses xy mask'''
    final_text_symbol = [(text, symbol) for text, symbol in filtered_text_symbol.items()]
    # final_text_symbol = []
    # for text, symbol in filtered_text_symbol.items():
    #     xt, yt, wt, ht, txt = text
    #     xs, ys, ws, hs = symbol
    #     slice_text=mask_xy[yt:yt+ht,xt:xt+wt]
    #     pos_text = cv2.findNonZero(slice_text)
    #     slice_symbol=mask_xy[ys:ys+hs,xs:xs+ws]
    #     pos_symbol = cv2.findNonZero(slice_symbol)
    #     if direction=='left' or direction=='right':
    #         slice_between=mask_xy[min(yt,ys):max(yt+ht,ys+hs),min(xt,xs):max(xt+wt,xs+ws)]
    #     if direction=='up':
    #         slice_between=mask_xy[ys+hs:yt,min(xt,xs):max(xt+wt,xs+ws)]
    #     pos_between = cv2.findNonZero(slice_between)
    #     if pos_between is None and pos_text is None and pos_symbol is None:
...
def remove_outliers(n_neighbors, direction, centers, sym_centers, text_symbol, txt_h, ratio):
    ...
    count = 0
    max_dist_thres = max(txt_h * 15, 2.5 * min(distances, key=lambda n: n[1])[1])
...
def calc_confusion_matrix(df, truth, pred, thres, txt_thres, rgb_original, text_color, symbol_color, plot):
...
def main(filename):
    ...
    # rgb,gray,ratio,rgb_original,filename = read_image(f'others/test_img/{filename}',resized_diagonal=2500)
    Height, Width = rgb.shape[:2]

    # get canny edge and filled
    canny = canny_filled(gray)
    canny_original = canny.copy()

    # delete long vertical lines
    mask_y = cv2.morphologyEx(canny, cv2.MORPH_OPEN, np.ones((91, 1), dtype=np.uint8), iterations=1)
    canny -= mask_y
    # delete long horizontal lines
    mask_x = cv2.morphologyEx(canny, cv2.MORPH_OPEN, np.ones((1, 99), dtype=np.uint8), iterations=1)
    canny -= mask_x
    ...
    # no 4+ (inclusive) l's
    is_text_criteria_loose = lambda conf, txt, h, txt_h: ((conf > conf_thres * 0.8 and
                                                           sum(c.isalpha() for c in txt) > 0 and
                                                           sum(c.isalnum() for c in txt) > 1) or
    ...
    for (x, y, w, h), results in recognition_results1.items():
        ...
        if is_text:
            '''text split'''
            x, w, symbols, recognized_text, results = text_split(x, y, w, h, gap_thres, results, symbols, symbol_color,
                                                                 image=rgb2,
                                                                 bw=canny,
                                                                 right_split=False,
    ...
    output, rgb3, direction = max((output_left, rgb3_left, 'left'), (output_right, rgb3_right, 'right'),
                                  (output_up, rgb3_up, 'up'), key=lambda n: len(n[0].keys()))
    if not output:
    ...
    df.columns = ['TP', 'FP', 'FN']
    rgb_TP_FP, rgb_FN = calc_confusion_matrix(df, truth, pred, thres, txt_thres, rgb_original, text_color, symbol_color,
                                              plot=True)
